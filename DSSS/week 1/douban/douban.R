...
session$current_url()
screenshot(session)
element <- session$find_element("xpath", "//div[@class = 'recent-hot-movie']")
...
Sys.sleep(2)
email_input$send_keys("jiaoyukun1999@gmail.com", keys$enter)
Sys.sleep(2)
password_input <- session$find_element("xpath", "//input[@type = 'password']")
Sys.sleep(2)
password_input$send_keys("DSSS2025DSSS2025", keys$enter)
Sys.sleep(2)
...
# Now on the article listing page!
...
ua <- user_agent("DSSS student project/1.0 (MacOS; only for course assignment; contact: yukji739@student.liu.se)")
...
{
  paste0("https://api.springernature.com/metadata/json?q=doi:", doi, "&api_key=e850606e3ae9999a24b9dfdf3be03af7")
})
