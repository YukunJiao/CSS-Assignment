...
to setup
  ...
  [
    nw:generate-watts-strogatz turtles links num_nodes 3 0.1 [set color green fd 10]
  ]

  if (network = "scale free")
  [
    nw:generate-preferential-attachment turtles links num_nodes 2 [set color green fd 10]
  ]

  if (network = "random")
  [
    nw:generate-watts-strogatz turtles links num_nodes 3 0.9 [set color green fd 10]
  ]
  ...
  ask turtles [
    ...
    if ( weight_type = "betweenness" ) [ set weight nw:betweenness-centrality / 50 ]
...
to go
  ...
  [
    ask turtles with [ color = red ] [ infect infect_triad ]
    tick
  ]
  [
...
to infect_triad
  ...
  let weight_list []
  let normalized_weight_list []
  let num_who []
  let influence_list []
  ask infected_neighbors
  [
    let influence ([ normalized_weight ] of self - [threshold_output] of self)
    set num_who lput [who] of self num_who
    set influence_list lput influence influence_list
  ]

  let temp position max influence_list influence_list
...
to normalize-sizes-and-colors
  if count turtles > 0 [
    ...
    ask turtles [ set color scale-color green size 0 5 ] ; using a higher range max not to get too white...
  ]
end
